package com.org.gnos.services.endpoints;

import static com.org.gnos.services.util.JsonUtil.json;

import static spark.Spark.delete;
import static spark.Spark.get;
import static spark.Spark.post;
import static spark.Spark.put;
import spark.Request;
import spark.Response;
import spark.Route;

import com.google.gson.JsonElement;
import com.google.gson.JsonObject;
import com.google.gson.JsonParser;
import com.org.gnos.services.common.ResponseError;
import com.org.gnos.services.controller.CapexController;

public class CapexEndpoint {

		CapexController controller;
		
		public CapexEndpoint() {
			controller = new CapexController();
			
			get("/scenario/:id/capex", (req, res) -> controller.getAll(req.params(":id")), json());
			
			post("/scenario/:id/capex", new Route() {
				
				@Override
				public Object handle(Request req, Response res) throws Exception {
					JsonElement requestObject = new JsonParser().parse(req.body());
					if(requestObject.isJsonObject()) {
						JsonObject jsonObject = requestObject.getAsJsonObject();
						try {
							return controller.create(jsonObject, req.params(":id"));
						} catch (Exception e) {
							res.status(400);
							return new ResponseError("Capex creation failed. "+e.getMessage());
						}					
					}
					res.status(400);				
					return new ResponseError("Capex creation failed due to improper input");
				}
			}, json());
			
	        put("/capex/:id", new Route() {
				
				@Override
				public Object handle(Request req, Response res) throws Exception {
					JsonElement requestObject = new JsonParser().parse(req.body());
					if(requestObject.isJsonObject()) {
						JsonObject jsonObject = requestObject.getAsJsonObject();
						try {
							return controller.update(jsonObject, req.params(":id"));
						} catch (Exception e) {
							res.status(400);
							return new ResponseError("Capex update failed. "+e.getMessage());
						}					
					}
					res.status(400);				
					return new ResponseError("Capex update failed due to improper input");
				}
			}, json());

			delete("/capex/:id", (req, res) -> controller.delete(req.params(":id")), json());
			
			delete("/capex/:id/capexInstance/:instanceid", (req, res) -> controller.deleteCapexInstance(req.params(":instanceid")), json());
		}
		
}
