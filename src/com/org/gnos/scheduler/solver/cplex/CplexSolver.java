...
public class CplexSolver implements ISolver {
	...
	public void solve(String fileName, int timePeiod) {
		...
		try {
			 ...
			 * + fileName); cplex.importModel(fileName);
			 */
	...
	public static Record parse(String x, int timePeiod) {
		...
		int spos = x.indexOf("s", 2); // stockpile block, look for "s" after
										// position 1 as "s" at pos 1 = reclaim
		...
		if (xpos != -1) { // all variables except binaries
			...
			/*
			 * else if(dpos == 0){ //it's a dump reclaim variable for global
			 * mode System.out.println("dc: " +x); //int prpos=x.indexOf("p",
			 * 2); //look for 2nd "p", first p is at 1.
			 * block=x.substring(xpos+1, wpos); pit=x.substring(1, xpos);
			 * process=x.substring(wpos,tpos); period=x.substring(tpos+1);
