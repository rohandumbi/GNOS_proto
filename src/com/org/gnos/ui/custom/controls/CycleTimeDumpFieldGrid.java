...
public class CycleTimeDumpFieldGrid extends Composite {
	...
	public CycleTimeDumpFieldGrid(Composite parent, int style) {
		super(parent, style);
	...
	private String[] getDumpFields(){
		int i = 0;
		int sourceFieldSize = this.dumpList.size();
		String[] dumps = new String[sourceFieldSize];
		for(i=0; i<sourceFieldSize; i++){
			dumps[i] = this.dumpList.get(i).getName();
		}
		return dumps;
	}
	...
	private void addRow(final String key){
		...
		comboSourceField.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				...
				dumpFieldMap.put(key, mappedFieldName);
			}
		});
		String existingMapping = dumpFieldMap.get(key);
	...
	private void createContent(Composite parent){
		...
		this.createHeader();
		for(String field: this.getDumpFields()){
			this.addRow(field);
		}
	}
